...
def build_faiss_index(
    ...
    if index_type == "Flat":
        ...
        index.nprobe = nprobe  # search nprobe clusters at query time
...
def load_index(save_dir: Path, nprobe: int = 10):
    index = faiss.read_index(str(save_dir / "item_index.faiss"))

    if hasattr(index, "nprobe"):
        index.nprobe = nprobe  # must reset after loading
...
def build_and_save_index(
    ...
    for start in range(0, len(feat_matrix), batch_size):
        batch = torch.tensor(
            feat_matrix[start:start + batch_size],
            dtype=torch.float32
        ).to(device)
    ...
    index = build_faiss_index(
        all_embeddings, index_type, nlist, nprobe
    )
