...
def evaluate_model(
    ...
    for user_id in val_users:
        ...
        # Reshape history → (1, max_history_len, history_embed_dim)
        hist_flat = np.array(
            user_feat_idx.loc[user_id, "user_history_embs"],
            dtype=np.float32
        )
        ...
        for k in k_values:
            metrics[f"ndcg_at_{k}"].append(
                compute_ndcg_at_k(relevant, ranked, k)
            )
